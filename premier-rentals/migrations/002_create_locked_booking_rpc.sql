...
begin
  ...
  returning bookings.id, bookings.total_amount, bookings.locked_until;
end;
$$;

revoke all on function public.create_locked_booking(
  uuid, text, date, text, text, text, text, text, text, text, text, text, text, text, text, integer, integer, numeric, text, timestamptz
) from public;
